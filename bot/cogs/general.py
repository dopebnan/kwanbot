...
class General(commands.Cog, name="general"):
    ...
    async def info(self, context):
        ...
        embed.set_footer(text=f"Time spent coding: 12hrs")
    ...
    async def check(self, context):
        ...
        embed = discord.Embed(title="check", color=0x4000D0)
        embed.add_field(name="r!pic images:", value=f"{pic_num}")
        embed.add_field(name="r!uwu images:", value=f"{uwu_num}")
        embed.add_field(name="Time since last update", value=f"<t:{version['lastUpdate']}:R>")
    ...
    async def uwu(self, context):
        if context.channel.id in config["uwuChannel"] or settings["uwu"]:
            ...
            if cum == 1:
                ...
                await context.send(random.choice(
                    ["uwu", "UwU", "OwO", "ÒwÓ", "ÙwÚ", "ÓwÒ", "^w^", ":3"]),
            ...
            if settings["picReturn"]:
                await context.send(
                    f"*To use this command outside <#884924639346823218> wait {settings['uwuCooldown']} seconds..*"
                )
